...
public class Descuentosdeldia {
    
      public void Listadedescuentos(ArrayList columna1, ArrayList columna2,ArrayList columna3, ArrayList columna4){
       ...
       //  auxs+= "PROP.JOSE MIGUEL CASTREZANA B.\n";  System.out.println("PROP.JOSE MIGUEL CASTREZANA B.\n");
        ...
        System.out.println("Fecha: " + dateFormat.format(date) + " Hora: " + hourFormat.format(date) + "\n");
         ...
         auxs+= "\n==============================\n";        System.out.println("\n==============================\n");
          auxs+= "Nombre  Sub  Desc  Tot\n";   System.out.println("Nombre  Sub  Desc  Tot\n");       
      System.out.println("\n==============================\n");
         auxs+= "\n==============================\n";                   
/*   if (descripcion.length() > 20) { // si la descripcion_producto es mayor a 17 la corta
   descripcion = descripcion.substring(0, 17);
      ...
      for(int n=0;n<=columna1.size()-1;n++){  
          String a =String.valueOf(columna1.get(n));
           if (a.length()> 12) { // si la descripcion_producto es mayor a 17 la corta
                a = a.substring(0, 12);           
            }
                auxs+= String.format("%-13s" + " " + "%-3s"+"  "+"$%-4s"+"  "+"$%-4s\n", a, columna2.get(n), columna3.get(n), columna4.get(n)); 
                System.out.println(String.format("%-13s" + " " + "%-3s"+"  "+"$%-4s"+"  "+"$%-4s", a, columna2.get(n), columna3.get(n), columna4.get(n)));      
  gastossumatoria+= Float.parseFloat(columna3.get(n).toString());
             auxs+= "\n";            
